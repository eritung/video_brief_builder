...
這是 V1.2 prototype；重複句非常多、多人同時說話、或重剪幅度很大的素材仍可能需要人工確認。
...
## V1.2 新增
- 產生需求頁面前，新增「台詞分段確認」步驟；只以字幕 A 為基準，可先合併、拆分、修改台詞分段。
- 可在分段階段直接勾選「這句另起一頁」。
- 需求編輯可「在此後插入新頁」或在尾端新增獨立頁。
- 編輯器、預覽與 PPTX 以字幕 A 的時間順序為主要排序，不再把移除等狀態集中到最後。
